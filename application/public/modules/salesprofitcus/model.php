 ...
 * @copyright 2015
 */
 class SalesprofitcusModel extends CI_Model{
	...
	function chi($search){
		...
		if(!empty($search['datetime'])){
			...
			$sql.= " and p.datecreate >= '".$fromdate."' ";	
			$sql.= " and p.datecreate <= '".$todate."' ";	
		}
		if(!empty($search['goodsid'])){
			$sql.= " and i.goodsid in (".$search['goodsid'].") ";	
		}	
		else{
			$sql.= " and i.goodsid in (0) ";	
		}
		/*if(!empty($search['customerid'])){
		...
		$sql1 = "
			select group_concat(t.uniqueid) as uniqueid
				from (
				SELECT p.uniqueid
				FROM `".$this->base_model->tb_input_createorders()."` p
				left join `".$this->base_model->tb_input()."` i on p.uniqueid = i.uniqueid
				where p.isdelete = 0
				$sql
				and i.isdelete = 0
				group by p.uniqueid
			...
			;
		";
		$query1 = $this->model->query($sql1)->execute();
		$listUninit = '';
		if(!empty($query1[0]->uniqueid)){
			$listUninit = $query1[0]->uniqueid;
		}
		//SQL Doanh thu
		$sql2 = '';
		$sql2.= " and ic.companyid = '".$companyid."' ";	
		if(!empty($branchid)){
			$sql2.= " and ic.branchid in (".$branchid.") ";	
		}
		if(!empty($search['datetime'])){
			$arrDate = explode('-',$search['datetime']);
			$fdates = trim($arrDate[0]);
			...
			$sql2.= " and p.datecreate >= '".$fromdate."' ";	
			$sql2.= " and p.datecreate <= '".$todate."' ";	
		}
		/*if(!empty($search['customerid'])){
			$sql2.= " and ic.customerid in (".$search['customerid'].") ";	
		}*/
		if(!empty($search['branchid'])){
			$sql2.= " and ic.branchid in (".$search['branchid'].") ";	
		}
		if(!empty($listUninit)){
			$sql2.= " and ic.uniqueid in (".$listUninit.") ";	
		}
		
		$query2s = "
			SELECT sum(p.amount) as total, date_format(p.datecreate,'%d-%m-%Y') as datecreate
			FROM `".$this->base_model->tb_pay()."` p
			left join `".$this->base_model->tb_input_createorders()."` ic on ic.uniqueid = p.uniqueid
			where p.isdelete = 0
			$sql2
			group by date_format(p.datecreate,'%d-%m-%Y')
			order by p.datecreate asc
			;
		";
		$data = $this->model->query($query2s)->execute();
		$arr = array();
		foreach($data as $item){
			$arr[$item->datecreate] = $item->total;
		}
		return $arr;
	}
	function thu($search){
		...
		$sql1 = "
			select group_concat(t.uniqueid) as uniqueid
				from (
				SELECT p.uniqueid
				FROM `".$this->base_model->tb_output_createorders()."` p
				left join `".$this->base_model->tb_output()."` i on p.uniqueid = i.uniqueid
				where p.isdelete = 0
				$sql
				and i.isdelete = 0
				group by p.uniqueid
		...
		$query2s = "
			SELECT sum(p.amount) as total, date_format(p.datecreate,'%d-%m-%Y') as datecreate
			FROM `".$this->base_model->tb_receipts()."` p
			left join `".$this->base_model->tb_output_createorders()."` ic on ic.uniqueid = p.uniqueid
			where p.isdelete = 0
			$sql2
			group by date_format(p.datecreate,'%d-%m-%Y')
			order by p.datecreate asc
