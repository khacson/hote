 ...
 * @copyright 2015
 */
class Salesprofitcus extends CI_Controller {
    private $route;
	private $login;
	...
	function thuChi(){
		...
		$searchs = json_decode($this->input->post('search'),true);
		$chis = $this->model->chi($searchs);
		$thus = $this->model->thu($searchs);
		$arr_date = array();
		foreach($chis as $k=>$v){
			$arr_date[$k] = 1;
		}
		...
		$categories = '';
		foreach($arr_date as $d=>$t){
			$categories.= ",'".$d."'";
		}
		if(!empty($categories)){
			$data->categories = substr($categories,1);
		}
		else{
			$data->categories = '';
		}
