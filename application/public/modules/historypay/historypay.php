 ...
 * @copyright 2015
 */
class Historypay extends CI_Controller {
    private $route;
	private $login;
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/lichsuchi.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(3,$i,$item->pay_code);
			...
			$sheetIndex->setCellValueByColumnAndRow(7,$i,$item->pay_type_name);
		...
		//$endxls = '.xls';
        $this->excel_model->exportExcel($objPHPExcel, $versionExcel, "Lichsuchi".$date.'.xls');
	}
