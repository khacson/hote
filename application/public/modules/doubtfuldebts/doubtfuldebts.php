 ...
 * @copyright 2015
 */
class Doubtfuldebts extends CI_Controller {
    private $route;
	private $login;
	...
	function _view(){
		...
		$data->orders = $this->model->getSalesOutput($data->companyid,$data->branchid,0);
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/congnobanhang.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(3,$i,$item->warehouse_name);
			...
			$sheetIndex->setCellValueByColumnAndRow(10,$i,$item->customer_phone);
			$sheetIndex->setCellValueByColumnAndRow(11,$i,$maturitydate);
			$sheetIndex->setCellValueByColumnAndRow(12,$i,$payments_status);
			$sheetIndex->setCellValueByColumnAndRow(13,$i,$item->employee_code .' - '.$item->employee_name);
		...
		//$endxls = '.xls';
        $this->excel_model->exportExcel($objPHPExcel, $versionExcel, "noxau_".$date.'.xls');
	}
