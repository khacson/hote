 ...
 * @copyright 2018
 */
class Ownertype extends CI_Controller {
    private $route;
	private $login;
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('loai-hinh-so-huu'),true);
        $this->site->render();
	}
	function getList(){
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hotel_customerownertype']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['ownertype_name'];
		$this->base_model->addAcction(getLanguage('loai-hinh-so-huu'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['ownertype_name'];
		$this->base_model->addAcction(getLanguage('loai-hinh-so-huu'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['hotel_customerownertype'])->save($id, $array);	
		
		$queryDelete = $this->model->table($tb['hotel_customerownertype'])
					->select('group_concat(ownertype_name) as deletes')
					->where("id in ($id)")
					->find();
