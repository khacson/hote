 ...
 * @copyright 2015
 */
class Inputreturn extends CI_Controller {
    private $route;
	private $login;
	...
	function _view(){
		...
		$data->uniqueid = $this->base_model->getUniqueid();
	...
	function edit(){
		...
		$array['percent']  = $percent;
		$array['description']  = $description;
		$arr = $this->model->edits($uniqueid,$array);
		$result['status'] = $arr['uniqueid'];
		$result['poid'] = $arr['poid'];
		$result['msg'] = $arr['msg'];
		if(empty($arr['msg'])){
			$result['uniqueidnew'] = $arr['uniqueid'];
		}
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	...
	function getDataPrintPX(){
		...
		$data->finds = $query[0];
		$result->content = $this->load->view('phieunhap', $data, true);
		echo json_encode($result);
	}
	...
	function formEditSO(){
		...
		if(empty($details[0]->orderid)){
			redirect('inputreturn');
		}
		$orderid = $details[0]->orderid;
		$data->soid = $soid;
		$finds = $this->model->findOrderUniqueidSO($orderid);
		$data->datas = $this->model->getTempGood($userid,$isnew,$soid);
		$data->customers = $this->base_model->getCustomer($finds->customer_id);
		$data->employeesale = $this->base_model->getEmployeesale($finds->employeeid);	
	...
	function getGoods(){
		...
		$findgoods = $this->model->table($tb['hotel_goods'])
								 ->select('id,goods_code,img,discounthotel_dly,discounthotel_type_dly,sale_price')
	...
	function updatePriceOne(){
		...
		$xkm = $this->input->post('xkm');
		$isnew = $this->input->post('isnew');
		$data = $this->model->updatePriceOne($goodid,$priceone,$quantity,$discount,$xkm,$isnew);
		$result = new stdClass();
