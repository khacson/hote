...
<tr class="tgrid" url= '<?=base_url();?>files/goods/<?=$item->img;?>'>
	...
	<td align="left">
		...
		<?php }?> (<?=$item->tonkho;?> <?=$item->unit_name;?>)
	...
	<td width="70"><input satid = "<?=$item->satid;?>" goodid="<?=$item->id;?>" uniqueid="<?=$item->uniqueid;?>" type="number" name="quantity" id="quantity_<?=$item->id;?>"  class="search form-control quantity text-right fm-number tab-event" value="<?=$item->quantity;?>" style="font-size:12px;" max="<?=$item->quantity;?>" min="0"  /></td>
	<td width="100">
		<?php 
	...
	<td>
		<input goodid="<?=$item->id;?>" type="text" id="xkm_<?=$item->id;?>" placeholder="" class="search form-control xuatkhuyenmai text-right tab-event" value="<?=fmNumber($item->xkm);?>" style="font-size:12px; float:left; width:100%;" max="<?=$item->xkm;?>"/>
	</td>
	<?php 
