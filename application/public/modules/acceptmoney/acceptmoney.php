...
class Acceptmoney extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('nhan-tien-ban-giao'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_acceptmoney']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$this->base_model->addAcction(getLanguage('nhan-tien-ban-giao'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$this->base_model->addAcction(getLanguage('nhan-tien-ban-giao'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['hotel_acceptmoney'])
