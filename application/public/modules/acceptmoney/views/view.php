...
<div class="portlet box blue mtop0">
	<div class="portlet-title">
		<div class="caption caption2">
			 <div class="brc mtop3"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('nhan-tien-ban-giao');?></div>			
		</div>
		<div class="tools">
					...
					<div id="tHeader">    	
						<table id="tbheader" width="100%" cellspacing="0" border="1" >
							...
							<tr>
								...
								<th id="ord_u.fullname"><?=getLanguage('nguoi-nhan');?></th>
								<th id="ord_u.fullname"><?=getLanguage('nguoi-giao');?></th>
								<th id="ord_ac.money"><?=getLanguage('so-tien');?></th>
								<th id="ord_ac.datecreate"><?=getLanguage('ngay-ban-giao');?></th>
								<th id="ord_ac.description"><?=getLanguage('ghi-chu');?></th>
								<th id="ord_ac.branchid"><?=getLanguage('chi-nhanh');?></th>
								<th></th>
								<th></th>
							</tr>
						</table>
					...
					<div id="gridView">
						<table id="tbbody" width="100%" cellspacing="0" border="1">
							...
							<tr class="row-search">
								...
								<td></td>
								<td>
									<select name="useraceptid" id="useraceptid"  class="combos" >
								...
								<td>
									<select name="personid" id="personid"  class="combos" >
										<?php foreach ($userAccepts as $item) { 
											?>
											<option value="<?=$item->id;?>"><?=$item->fullname;?></option>
										<?php } ?>
								...
								<td>
									<input type="text" name="money" id="money" class="searchs form-control " />
								</td>
...
<script>
	...
	function searchFunction(){
		$("#money,#description").keyup(function() {
			searchList();	
		});
	...
	function save(){
		...
		if(obj.pay_type_name == ''){
			warning("<?=getLanguage('nhan-tien-ban-giao')?>"); return false;	
		}
		var token = $('#token').val();
		$('.loading').show();
		$.ajax({
	...
	function init(){
		...
		$('#personid').multipleSelect({
			filter: true,
			single: false,
			placeholder: "<?=getLanguage('chon-nguoi-giao')?>",
			onClick: function(view){
				searchList();
			}
		});
    ...
    function funcList(obj){
		$('.edit').each(function(e){
			$(this).click(function(){ 
				 var pay_type_name = $('.pay_type_name').eq(e).html().trim();
				 var id = $(this).attr('id');
				 $('#id').val(id);	
