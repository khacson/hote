...
class Customertype extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('phan-loai-khach-hang'),true);
        $this->site->render();
	}
	function getList(){
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hotel_customertype']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['customertype_name'];
		$this->base_model->addAcction(getLanguage('phan-loai-khach-hang'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['customertype_name'];
		$this->base_model->addAcction(getLanguage('phan-loai-khach-hang'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['hotel_customertype'])->save($id, $array);	
		
		$queryDelete = $this->model->table($tb['hotel_customertype'])
					->select('group_concat(customertype_name) as deletes')
					->where("id in ($id)")
					->find();
