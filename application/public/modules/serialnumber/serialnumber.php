...
class Serialnumber extends CI_Controller {
	...
	function save() {
		...
		$description = 'Thêm mới: '.$array['sn'];
		$this->base_model->addAcction('Serial',$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = 'Sửa: '.$array['sn'];
		$this->base_model->addAcction('Serial',$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['hotel_seial'])
					->select('group_concat(sn) as deletes')
					->where("id in ($id)")
					->find();
