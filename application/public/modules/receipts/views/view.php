					...
					<div id="tHeader">    	
						<table id="tbheader" width="100%" cellspacing="0" border="1" >
							...
							<tr>							
								...
								<th id="ord_p.supplierid"><?=getLanguage('khach-hang')?></th>		
...
<script>
	...
	$(function(){
		...
		$('#printpc').click(function(){
			var receipts_code = $('#print_pay_code').val();
			if(receipts_code == ''){
				warning("<?=getLanguage('chon-phieu-thu');?>");
			}
			inPhieuchi(receipts_code);
		});
		searchFunction();
	});
	function searchFunction(){
		$("#poid,#amount,#receipts_code").keyup(function() {
			searchList();	
		});
	...
	function inPhieuchi(receipts_code){
			$.ajax({
				url :  '<?=base_url();?>congnomuahang/getDataPrintPC',
				type: 'POST',
				async: false,
				data: {ptid:receipts_code},
    ...
    function funcList(obj){
		$('.edit').each(function(e){
			$(this).click(function(){ 
				 ...
				 $('#print_pay_code').val(receipts_code);	
