 ...
 class CreateordersModel extends CI_Model{
	...
	function saves($uniqueid,$itemList,$array,$isnew){
		...
		if($setuppo == 0){
			...
			$poid = $array['poid']; 
			//Kiem tra ton tai PO
			$checkPOid = $this->model->table($tb['hotel_orders'])
								 ->select('poid')
								 ->where('poid',$poid )
			...
			if(!empty($checkPOid->poid)){
				$status['uniqueid'] = -2;
				$status['poid'] = $poid;
				$status['msg'] = getLanguage('ma-don-hang-da-ton-tai');;
		...
		$insert['adjustment'] = fmNumberSave($array['adjustment']);
		...
		$this->model->table($tb['hotel_orders'])->insert($insert);	
		#end
		$orderid = $this->model->table($tb['hotel_orders'])	
							   ->select('id')
		...
		foreach($queryTemp as $item){
			...
			$this->model->table($tb['hotel_orders_detail'])->insert($insertInput);
			$i++;
		}
		#end
		...
		$this->base_model->addAcction(getLanguage('bao-gia'),$this->uri->segment(2),'','',$description );	
		#end
	...
	function edits($uniqueid,$itemList,$array,$isnew){
		...
		$insert['warehouseid'] = 0;//$array['warehouseid'];
		//print_r($insert); exit;
		$this->model->table($tb['hotel_orders'])->where('id',$orderid)->update($insert);	
		#end
		/*$orderid = $this->model->table($tb['hotel_orders'])	
							   ->select('id')
							   ->where('uniqueid',$uniqueid)
		...
		#region them bang detail
		//Xoa order id cũ
		$this->model->table($tb['hotel_orders_detail'])->where('orderid',$orderid)->delete();
		$i=1;
		foreach($queryTemp as $item){
			$insertInput = array();
			$insertInput['warehouseid'] = 0;
	...
	/*function edits($orderid,$array){
		...
		#end
		//Lay du lieu ban dau
		$queryOld = $this->model->table($tb['hotel_orders_detail'])
		...
		$this->model->table($tb['hotel_orders_detail'])
					->where('orderid',$orderid)
					->where('branchid',$branchid)
					->delete();
	...
	function findID($id){
		$tb = $this->base_model->loadTable();
		$query = $this->model->table($tb['hotel_orders_detail'])
					  ->select('*')
					  ->where('id',$id)
					  ->find();
		return $query;
	...
	function findListID($unit){
		$tb = $this->base_model->loadTable(); //customer_id
		$sql = "
			SELECT oo.id, so.vat, so.signature_x, so.signature_name_x, so.place_of_delivery, so.stt, so.poid, so.datepo, oo.sttview, oo.goodsid, oo.branchid, sum(oo.quantity) as quantity, oo.priceone, oo.price, so.description, sum(oo.cksp) as cksp,
			...
			un.unit_name, so.discount_value, so.discount_type, so.discount, so.adjustment, so.vat_value
			FROM `".$tb['hotel_orders']."` so
			LEFT join `".$tb['hotel_orders_detail']."` oo on oo.orderid = so.id
			left join `".$tb['hotel_customer']."` c on c.id = so.customerid and c.isdelete = 0
	...
	function getTempGood($userid,$isnew = 0){
		...
		$branchid = $this->login->branchid; //price
		$sql = "
				...
				if(sat.discount is not null,sat.discount,s.discounthotel_dly) as discount, s.isserial, sat.vat, sat.price_total, sat.discount_value,
	...
	function getNoneCreateorders(){
		...
		WHERE table_name='".$tb['hotel_orders']."'; 
		";
		$query = $this->model->query($sql)->execute();
		$obj = new stdClass();
		foreach($query as $item){
	...
	function findGoodsByPO($poid){
		...
		$companyid = $this->login->companyid;
		$skey = $this->login->skey;
		$find = $this->model->table($tb['hotel_orders'])
	...
	function createPoOrder($branchid,$datecreate){
		...
		$checkSTT = $this->model->table($tb['hotel_orders'])
					 ->select('poid,stt')
					 ->where("datepo like '$yearDay%'")
					 ->where('branchid',$branchid)
					 ->where('isdelete',0)
	...
	function getDiscountorderSO($goodid,$poid){
		...
		$sql = "
			SELECT g.id, sat.discount, g.goods_code, DES_DECRYPT(g.goods_name,'".$skey."') as goods_name
			FROM `".$tb['hotel_orders_detail_order']."`  sat
	...
	function findListUniqueID($orderid){
		...
		$branchid = $this->login->branchid;
		$sql = "
			select si.*, sg.goods_code, sg.goods_name, 0 as stype,
			(
				SELECT oc.vat
				FROM `".$tb['hotel_orders']."` oc
