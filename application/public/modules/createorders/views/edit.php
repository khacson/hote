...
<script>
	...
	function save(func,id){ 
		...
		objReqest['total_discount'] = $('#total-discount').val(); //total-discount
		...
		$.ajax({
			...
			success:function(datas){
				...
				if(obj.status == 0){
					...
					success("<?=getLanguage('sua-bao-gia-khong-thanh-cong');?> "+ obj.poid);
				}
			},
			error : function(){
				$(".loading").hide();
				error("<?=getLanguage('sua-bao-gia-khong-thanh-cong');?>"); return false;
			}
		});
	}
	...
	function gooods(goodsid,code,stype,exchangs,deletes){ 
		...
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {vat:vat,id:goodsid,code:code,stype:stype,exchangs:exchangs,deletes:deletes,isnew:isnew,uniqueid:uniqueid},
			success:function(datas){
				var obj = $.evalJSON(datas); 
	...
	function print(id){
		...
		$.ajax({
			...
			success:function(datas){
				var object = $.evalJSON(datas); 
				var disp_setting = "toolbar=yes,location=yes,directories=yes,menubar=no,";
			disp_setting += "scrollbars=yes,width=950, height=auto, left=0.0, top=0.0";
				var docprint = window.open("certificate", "certificate", disp_setting);
				docprint.document.open();
				docprint.document.write('<html>');
				//docprint.document.write(css);
