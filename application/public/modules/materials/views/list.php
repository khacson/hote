...
foreach ($datas as $key => $item) { 	
	if(!empty($array[$item->discounthotel_type])){
		$discounthotel_type = $array[$item->discounthotel_type];
	}
	else{
		$discounthotel_type = "";
	}
	if(!empty($array[$item->discounthotel_type_dly])){
	...
	discounthotel_type="<?=$item->discounthotel_type;?>"
	discounthotel_dly="<?=$item->discounthotel_dly;?>"
	discounthotel_type_dly="<?=$item->discounthotel_type_dly;?>"
	...
	>
		...
		<td class="discountsales text-right"><?=fmNumber($item->discountsales);?><?=$discounthotel_type;?></td> 
		<td class="discounthotel_dly text-right"><?=fmNumber($item->discounthotel_dly);?><?=$discounthotel_type_dly;?></td>
		<td class="description"><?=$item->description;?></td>
		<td class="text-right quantitymin" ><?=$item->quantitymin;?></td>
		<td class="shelflife" ><?=$shelflife;?></td>
