						...
						<table class="inputgoods">
							...
							<tbody class="gridView">
								...
								<tr >
									...
									<td class="ttprice" style="text-align:right; padding-right:10px !important; font-size:12px;"><?=fmNumber($find->price);?></td>
									<td></td>
									<td></td>
							...
							<div class="form-group">
								<label class="control-label col-md-4">Ngày nhập</label>
								 <div class="col-md-8 input-group date date-picker" data-date-format="<?=cfdateHtml();?>">
									<input value="<?=date(cfdate(),strtotime($find->datecreate));?>" 
									type="text" id="datecreate" placeholder="<?=cfdateHtml();?>" name="datecreate" class="form-control searchs" >
									...
									<span id="loadWarehouse">
										<select id="warehouseid" name="warehouseid" class="combos" >
											...
											<?php foreach($warehouses as $item){?>
												<option <?php if($find->warehouseid == $item->id){?> selected <?php }?> value="<?=$item->id;?>"><?=$item->warehouse_name;?></option>
											<?php }?>
										</select>
									</span>
								</div>
						...
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									...
									<div class="col-md-4">
										<label class="control-label">
										<input <?php if($find->payments ==3){?> checked <?php }?> type="radio"  id="payments_3" name="payments" value="3"  />
										Thẻ</label>
									</div>
								</div>
							</div>
					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input type="text" name="input_total" id="input_total" readonly placeholder="" value="<?=fmNumber($find->price);?>" class="searchs form-control text-right fm-number" />
								</div>
							</div>
						</div>
					</div><!--E Row-->
							...
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									...
									<div class="col-md-5" style="padding:0 !important; ">
										<input style="font-size:12px;" type="text" name="price_prepay" id="price_prepay" placeholder="" class="searchs form-control text-right fm-number" value="<?=fmNumber($find->price_prepay);?>" />
									</div>
								</div>
							</div>
						</div>
					...
					<div class="row mtop10 mbottom10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input value="<?=fmNumber($find->price - $find->price_prepay);?>" type="text" name="price_indebtedness" id="price_indebtedness" readonly placeholder="" class="searchs form-control text-right fm-number" />
								</div>
							</div>
						</div>
					</div><!--E Row-->
...
<input id="uniqueid" name="uniqueid" value="<?=$find->uniqueid;?>" type="hidden" />
<!--E My form-->
<style type="text/css">
	.inputgoods{
		width:98%;	
...
<script>
		...
		$.ajax({
			...
			success:function(datas){
				...
				if(obj.status == 0){
					...
					$('#unit').val(obj.status);
					if(id == 0){
						toastr.success('Sửa nhập kho thành công.','Thông báo', {closeButton:true, timeOut:3000});
						inputList = {};
					}
					else{
						 toastr.success('Nhập kho thành công.','Thông báo', {closeButton:true, timeOut:3000});
						 inputList = {};
						 print(id,obj.status);
					}
	...
	function gooods(goodsid,code,stype,exchangs,deletes){ 
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {id:goodsid,code:code,stype:stype,exchangs:exchangs,deletes:deletes,isnew:1},
			success:function(datas){
				var obj = $.evalJSON(datas); 
	...
	function init(){
		...
		$( "#price_prepay" ).keyup(function(e){
			if ((e.keyCode >= 48 && e.keyCode <= 57) || (e.keyCode >= 96 && e.keyCode <= 105) || e.keyCode == 8 || e.keyCode == 46) { //0-9 only, backspace, delete
				...
				inputTotal = parseFloat(inputTotal.replace(/[^0-9+\-Ee.]/g, ''));
				var pricePrepay = $('#price_prepay').val();
				if(percent == 0){//Tiem mat
					pricePrepay = parseFloat(pricePrepay.replace(/[^0-9+\-Ee.]/g, ''));
				}
				else{//phan tram
					pricePrepay = parseFloat(pricePrepay.replace(/[^0-9+\-Ee.]/g, ''));	
					var perc = pricePrepay * inputTotal / 100; 
					pricePrepay = perc.toFixed(2);
				}
