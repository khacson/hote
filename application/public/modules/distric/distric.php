...
class Distric extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('quan-huyen'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns('hotel_district');
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
		...
		$data->provinces = $this->model->getProvice();
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['distric_name'];
		$this->base_model->addAcction(getLanguage('quan-huyen'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['distric_name'];
		$this->base_model->addAcction(getLanguage('quan-huyen'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table('hotel_district')
					->select('group_concat(distric_name) as deletes')
					->where("id in ($id)")
					->find();
