...
class Warehouse extends CI_Controller {
	...
	function _view(){
		...
		$data->iswarehouse = $this->login->count_warehouse;
		$data->isbranh = $this->login->count_branch;
		$data->count_warehouse = $login->count_warehouse;
		$data->total = $this->model->getTotal(array());
		...
		$this->site->write('title',getLanguage('kho-hang'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_warehouse']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$login = $this->login;
		$count_warehouse = $login->count_warehouse;
		$total = $this->model->getTotal(array());
		if($total < $count_warehouse){
			...
			$result['status'] = -2; 
			$result['count_warehouse'] = $count_warehouse; 
			echo json_encode($result);
			exit;
		}
		$description = getLanguage('them-moi').': '.$array['warehouse_name'];
		$this->base_model->addAcction(getLanguage('kho-hang'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['warehouse_name'];
		$this->base_model->addAcction(getLanguage('kho-hang'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function deletes() {
		...
		$checkExitInv = $this->model->table($tb['hotel_inventory'])
									  ->select('warehouseid')
									  ->where("warehouseid in ($id)")
									  ->where('isdelete',0)
									  ->find();
		if(!empty($checkExitInv->warehouseid)){
			$result['status'] = -1;	
			$result['csrfHash'] = $token;
			echo json_encode($result); exit;
		}
		#end
		$this->model->table($tb['hotel_warehouse'])
								->where("id in ($id)")
								->update($array);	
		
		$queryDelete = $this->model->table($tb['hotel_warehouse'])
					->select('group_concat(warehouse_name) as deletes')
					->where("id in ($id)")
					->find();
