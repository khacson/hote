 ...
 * @copyright 2018
 */
class Orderroomhistory extends CI_Controller {
    private $route;
	private $login;
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['roomtype_name'];
		$this->base_model->addAcction(getLanguage('lich-su-dat-phong'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['roomtype_name'];
		$this->base_model->addAcction(getLanguage('lich-su-dat-phong'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['hotel_roomtype'])
					->select('group_concat(roomtype_name) as bankname')
					->where("id in ($id)")
					->find();
		
		$this->model->table($tb['hotel_roomtype'])
					->select('group_concat(roomtype_name) as bankname')
