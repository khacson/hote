 ...
 * @copyright 2015
 */
class Historyreceipts extends CI_Controller {
    private $route;
	private $login;
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/lichsuthu.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(2,$i,$poid);
			$sheetIndex->setCellValueByColumnAndRow(3,$i,$item->receipts_code);
			$sheetIndex->setCellValueByColumnAndRow(4,$i,$item->amount);
			$sheetIndex->setCellValueByColumnAndRow(5,$i,$payment);
			$sheetIndex->setCellValueByColumnAndRow(6,$i,$datepo);
			$sheetIndex->setCellValueByColumnAndRow(7,$i,$item->receipts_type_name);
			$sheetIndex->setCellValueByColumnAndRow(8,$i,$item->notes);
		...
		//$endxls = '.xls';
        $this->excel_model->exportExcel($objPHPExcel, $versionExcel, "Lichsuthu".$date.'.xls');
	}
	function getDataPrintPC(){
		...
		$dataprint = $this->model->findUniqueID($uniqueid);
