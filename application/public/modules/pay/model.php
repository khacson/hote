 ...
 */
 class PayModel extends CI_Model{
	...
	function getList($search,$page,$rows){
		...
		$having = '';
		$sql = "SELECT p.*, sp.supplier_name, b.bank_name
				FROM `".$tb['hotel_pay']."` p
				LEFT JOIN `".$tb['hotel_bank']."` AS b on b.id = p.bankid 
				LEFT JOIN `".$tb['hotel_supplier']."` sp on sp.id = p.supplierid 
				WHERE p.isdelete = 0
				AND p.pay_type > 0
				$searchs
				";
	...
	/*function dsCongNo($array,$id,$payments,$datecreate){
		...
		$sql = "SELECT lb.id, lb.price, 
				...
				LEFT JOIN `".$tb['hotel_supplier']."` sp on sp.id = lb.supplierid 
				WHERE sp.isdelete = 0
	...
	function conNo($array,$id,$payments){
		...
		$tb = $this->base_model->loadTable();
		$having = '';
		$sql = "SELECT  lb.supplierid, lb.datecreate, lb.id, lb.price, lb.uniqueid,
				(
					SELECT sum(p.amount)
					FROM `".$tb['hotel_pay']."` p
					where p.isdelete = 0
		...
		if(empty($query[0]->id)){
			...
			$arr = array();
			$arr['supplierid'] = $query[0]->supplierid;
