...
class Location extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('vi-tri-luu-kho'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_location']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['location_name'];
		$this->base_model->addAcction(getLanguage('vi-tri-luu-kho'),$this->uri->segment(2),'','',$description );
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['location_name'];
		$this->base_model->addAcction(getLanguage('vi-tri-luu-kho'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$array['isdelete'] = 1;
		$array['userupdate'] = $this->login->username;
		//Kiểm tra vi tri da luu kho chua
		$tb = $this->base_model->loadTable();
		$checkLocation = $this->model->table($tb['hotel_inventory'])
							 ->select('id')
		...
		$this->model->table($tb['hotel_location'])
					->where('id in ('.$id.')')
					->update($array);	
		
		$queryDelete = $this->model->table($tb['hotel_location'])
					->select('group_concat(location_name) as deletes')
					->where("id in ($id)")
					->find();
