 ...
 * @copyright 2018
 */
class Activefields extends CI_Controller {
    private $route;
	private $login;
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('linh-vuc-hoat-dong'),true);
        $this->site->render();
	}
	function getList(){
	...
	function form(){
		...
		if(empty($find->id)){
			$find = $this->base_model->getColumns($tb['hotel_customeractivefields']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['activefields_name'];
		$this->base_model->addAcction(getLanguage('linh-vuc-hoat-dong'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['activefields_name'];
		$this->base_model->addAcction(getLanguage('linh-vuc-hoat-dong'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['hotel_customeractivefields'])->save($id, $array);	
		
		$queryDelete = $this->model->table($tb['hotel_customeractivefields'])
					->select('group_concat(activefields_name) as deletes')
					->where("id in ($id)")
					->find();
