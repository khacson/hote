 ...
 * @copyright 2015
 */
class Liabilitiessales extends CI_Controller {
    private $route;
	private $login;
	function __construct(){
		...
		$this->route = 'cong-no-ban-hang';//$this->router->class;
		$menus = $this->site->getSession('menus');
		$this->title = $menus['cong-no-ban-hang'];
	}
	...
	function save() {
		...
		$result['status'] =$this->model->saves($array,$idselect,$payments);
	...
	function edit() {
		...
		$result['status'] =$this->model->edits($array,$id,$payments);
	...
	function viewPOdetail(){
		...
		$data = new stdClass();
		$id = $this->input->post('id');
		$data->datas = $this->model->getListDetailPO($id);
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/liabilitiessales.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(6,$i,$item->price_total - $item->price_prepay);
			$sheetIndex->setCellValueByColumnAndRow(7,$i,$customer_type);
			$sheetIndex->setCellValueByColumnAndRow(8,$i,$item->customer_name);
			$sheetIndex->setCellValueByColumnAndRow(9,$i,$maturitydate);
			$sheetIndex->setCellValueByColumnAndRow(10,$i,$payments);
			$sheetIndex->setCellValueByColumnAndRow(11,$i,$item->employee_code .' - '.$item->employee_name);
			$sheetIndex->setCellValueByColumnAndRow(12,$i,$item->description);
			$sheetIndex->setCellValueByColumnAndRow(13,$i,date('d-m-Y H:i:s',strtotime($item->datecreate)));
			$i++;
		}
		$boderthin = "A2:N".($i-1); 
		...
		//$endxls = '.xls';
        $this->excel_model->exportExcel($objPHPExcel, $versionExcel, "congnobanhang_".$date.'.xls');
	}
}