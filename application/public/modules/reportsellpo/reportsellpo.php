 ...
 * @copyright 2015
 */
class Reportsellpo extends CI_Controller {
    private $route;
	private $login;
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/reportsellpo.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(7,$i,($item->price_prepay));
			$sheetIndex->setCellValueByColumnAndRow(8,$i,($item->price - $item->price_prepay));
			$i++;
		}
		$boderthin = "A2:I".($i-1); 
		...
		//$endxls = '.xls';
        $this->excel_model->exportExcel($objPHPExcel, $versionExcel, "banhangtheodonhang_".$date.'.xls');
	}
	function getGoods(){
