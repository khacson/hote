 ...
 * @copyright 2015
 */
 class ReportliabilitiessellModel extends CI_Model{
	...
	function getList($search,$page,$rows){
		...
		$sql = "SELECT c.*, s.goods_code, inc.payments_status, inc.customer_phone,  
		DES_DECRYPT(s.goods_name,'".$this->login->skey ."') as goods_name, w.warehouse_name, 
		DES_DECRYPT(cm.customer_name,'".$this->login->skey ."') as customer_name, 
		ut.unit_name, e.employee_code ,
		DES_DECRYPT(e.employee_name,'".$this->login->skey ."')as employee_name, 
		s.unitid, inc.payments_status as payments_statuss, inc.payments, inc.customer_type, inc.description  
				FROM hotel_output AS c
				LEFT JOIN hotel_output_createorders inc on inc.uniqueid = c.uniqueid 
				...
				LEFT JOIN hotel_customer cm on cm.id = inc.customer_id and cm.isdelete = 0
				LEFT JOIN hotel_unit ut on ut.id = s.unitid and ut.isdelete = 0 
				LEFT JOIN hotel_employeesale e on e.id = inc.employeeid and e.isdelete = 0 
				WHERE c.isdelete = 0 
				AND inc.isout = 1
				AND inc.payments_status = -1
				$searchs
				";
