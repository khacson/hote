...
class Rawdata extends CI_Controller {
	...
	function _view(){ 
		...
		$this->site->write('title',getLanguage('nguyen-phu-lieu'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tb['hotel_rawdata']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['rawdata_name'];
		$this->base_model->addAcction(getLanguage('nguyen-phu-lieu'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['rawdata_name'];
		$this->base_model->addAcction(getLanguage('nguyen-phu-lieu'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['hotel_rawdata'])
					->select('group_concat(rawdata_name) as deletes')
					->where("id in ($id)")
					->find();
