...
class Createorders extends CI_Controller {
	...
	function save(){
		...
		$arr = $this->model->saves($uniqueid,$array);
	...
	function getDataPrintPX(){
		...
		$data->finds = $query[0];
		$result->content = $this->load->view('dondathang', $data, true);
		echo json_encode($result);
	}
	...
	function getGoods(){
		...
		$findgoods = $this->model->table($tb['hotel_goods'])
								 ->select('id,unitid,goods_code,img,discounthotel_dly,discounthotel_type_dly,sale_price')
