					...
					<div class="row" style="margin-bottom:15px;">
						<div class="row">
							<div class="form-group">
								...
								<!--<div class="col-md-4" style="padding-left:0; margin-left:-3px; display:none">
					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 ">
									<input type="text" name="description" id="description" placeholder="" value="" class="searchs form-control tab-event" />
								</div>
							</div>
						</div>
					</div><!--E Row-->
...
<script>
	...
	function save(func,id){ 
		...
		$.ajax({
			...
			success:function(datas){
				...
				if(obj.status == 0){
					...
					if(id == 0){
						...
						success('Sửa xuất hàng thành công.');
					}
					else{
						$('#unit').val(obj.status);
						$('#poid').val(obj.poid);
			...
			error : function(){
				$(".loading").hide();
				error('Sửa xuất hàng không thành công.'); return false;
			}
		});
	}
	...
	function gooods(goodsid,code,stype,exchangs,deletes){ 
		...
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {vat:vat, id:goodsid,code:code,stype:stype,exchangs:exchangs,deletes:deletes,isnew:1,uniqueid:uniqueid},
			success:function(datas){
				var obj = $.evalJSON(datas); 
				if(obj.status == 0){
	...
	function setDefault(){
		...
		var datecreate = '<?=$finds->datecreate;?>';
		var deliverydate = '<?=$finds->deliverydate;?>';
	...
	function updateDataTemp(goodid){
		...
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {goodid:goodid, priceone:priceone,quantity:quantity,discount:discount,isnew:1},
			success:function(datas){
				var object = $.evalJSON(datas); 
	...
	function updateSerial(goodid,serial_number){
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {goodid:goodid, serial_number:serial_number,isnew:1},
			success:function(datas){}
		}); 
		//calInputTotal();
	}
