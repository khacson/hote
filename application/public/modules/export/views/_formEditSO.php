						...
						<div class="row">
							<div class="form-group">
								...
								<div class="col-md-4" style="padding-left:0; margin-left:-3px;">
									<select class="combos tab-event" id="oderpoid" name="oderpoid">
										<option value=""></option>
						...
						<table class="inputgoods">
							<thead>
								...
								<!--<tr class="thd">
									<td width="30">STT</td>
									<td >Hàng hóa</td>
									<td width="85">Số lương</td>
									<td width="90">Đơn giá</td>
									<td width="100">Thành tiền</td>
									<td width="100">Chiết khấu</td>
									<?php if(isguarantee() == 1){?>
									<td width="110">Hạn bảo hành</td>
									<?php }?>
									<?php if(isserial() == 1){?>
							...
							<div class="form-group">
								...
								<div class="col-md-7">
									<span id="loadCustomer">
										<select id="customerid" name="customerid" class="combos tab-event" >
											<option value=""></option>
											<?php foreach($customers as $item){
												if($item->id != $finds->customer_id){
													continue;
												}
												?>
						...
						<div class="col-md-12">
							<div class="form-group">
								<label class="control-label col-md-4">Ngày giao</label>
								 <div class="col-md-8 input-group date date-picker" data-date-format="<?=cfdateHtml();?>">
									<input value="<?=$datecreate;?>" type="text" id="deliverydate" placeholder="<?=cfdateHtml();?>" name="deliverydate" class="form-control searchs tab-event" >
									<span class="input-group-btn ">
										<button class="btn default btn-picker" type="button"><i class="fa fa-calendar "></i></button>
									</span>
								</div>
							...
							<div class="form-group">
								<label class="control-label col-md-4">Hạn bảo hành</label>
								 <div class="selectAllGuarantee col-md-8 input-group date date-picker" data-date-format="<?=cfdateHtml();?>">
									...
									<input value="" type="text" id="guarantee" placeholder="<?=cfdateHtml();?>" name="guarantee" class="form-control searchs tab-event" >
									<span class="input-group-btn ">
										<button class="btn default btn-picker" type="button"><i class="fa fa-calendar "></i></button>
									</span>
								</div>
					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 ">
									<input type="text" name="place_of_delivery" id="place_of_delivery" placeholder="Địa điểm giao hàng" value="" class="searchs form-control tab-event" />
								</div>
							</div>
						</div>
					</div>
...
<script>
	...
	function save(func,id){ 
		...
		var uniqueid = '<?=$uniqueid;?>';$('#uniqueidnew').val();
		// return false;
		$(".loading").show();
		...
		var soid = '<?=$soid;?>';
		$.ajax({
			url : controller + 'saveso',
			type: 'POST',
			async: false,
			data: {csrf_stock_name:token,search:JSON.stringify(obj),description:description,uniqueid:uniqueid, price_prepay:price_prepay,payments:payments,prepay:prepay,soid:soid},
			success:function(datas){
				...
				if(obj.status == 0){
					...
					if(id == 0){
						...
						success('Xuât bán hàng thành công.');
					}
					else{
						$('#unit').val(obj.status);
						$('#poid').val(obj.poid);
			...
			error : function(){
				$(".loading").hide();
				error('Xuât bán không thành công.'); return false;
			}
		});
	}
	...
	function gooods(goodsid,code,stype,exchangs,deletes){ 
		...
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {vat:vat, id:goodsid,code:code,stype:stype,exchangs:exchangs,deletes:deletes,isnew:2,uniqueid:uniqueid},
			success:function(datas){
				var obj = $.evalJSON(datas); 
				if(obj.status == 0){
	...
	function setDefault(){
		var employeeid = '<?=$finds->employeeid;?>';
		var customer_id =  '<?=$finds->customer_id;?>';
		var datecreate = '<?=date(cfdate(),strtotime($finds->datecreate));?>';
		var deliverydate = '<?=date(cfdate(),strtotime($finds->deliverydate));?>';
		var place_of_delivery = '<?=$finds->place_of_delivery;?>';
		var description = '<?=$finds->description;?>';
		var price_total = '<?=$finds->price_total;?>';
		var soid = '<?=$soid;?>';
		var vat =  '<?=$finds->vat;?>';
		$('#customerid').multipleSelect('setSelects',[customer_id]);
		$('#employeeid').multipleSelect('setSelects',[employeeid]);
		$('#oderpoid').multipleSelect('setSelects',[soid]);
		$('#datecreate').val(datecreate);
		$('#deliverydate').val(deliverydate);
		$('#place_of_delivery').val(place_of_delivery);
		$('#description').val(description);
		$('#vat').val(vat);
		var payments =  '<?=$finds->payments;?>';
		$('#prepay_1').prop('checked', true);
		$('#payments_'+payments).prop('checked', true);
		//$('#input_total').val(price_total);
	}
	function updateDataTemp(goodid){
		...
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {goodid:goodid, priceone:priceone,quantity:quantity,discount:discount,isnew:2},
			success:function(datas){
				var object = $.evalJSON(datas); 
	...
	function updateSerial(goodid,serial_number){
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {goodid:goodid, serial_number:serial_number,isnew:2},
			success:function(datas){}
		}); 
		//calInputTotal();
	}
	function init(){
		...
		$('#oderpoid').multipleSelect({
			...
			onClick: function(view) {
				var soid = getCombo('oderpoid');
				window.location = '<?=base_url();?>export/formEditSO?soid='+soid;
			}
		});
