					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 ">
									<input type="text" name="description" id="description" placeholder="" value="" class="searchs form-control " maxlength="250"/>
								</div>
							</div>
						</div>
					</div><!--E Row-->
...
<script>
	...
	$(function(){
		...
		$('#viewPrint').click(function(){
			var unit = $('#unit').val();
			if(unit == ''){
				warning("<?=getLanguage('in-khong-thanh-cong');?>");
				return false;
			}
			printSave(0,unit);
		});
		...
		$('#price_prepay').click(function(e){
			 var totalamountend = $('#total-amount-end').val();
			 $('#price_prepay').val(totalamountend);
			 $('#price_prepay').select();
			  calInputTotal();
		});
	...
	function save(func,id){ 
		...
		objReqest['oderpoid'] = $('#oderpoid').val();
		...
		$.ajax({
			...
			success:function(datas){
				...
				if(obj.status == 0){
					...
					success("<?=getLanguage('xuat-ban-thanh-cong');?> "+ obj.poid);
				}
			},
			error : function(){
				$(".loading").hide();
				error("<?=getLanguage('xuat-ban-khong-thanh-cong');?>"); return false;
			}
		});
	}
