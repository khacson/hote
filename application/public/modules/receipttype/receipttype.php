 ...
 * @copyright 2015
 */
class Receipttype extends CI_Controller {
    private $route;
	private $login;
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('loai-phieu-thu'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_receipts_type']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['receipts_type_name'];
		$this->base_model->addAcction(getLanguage('loai-phieu-thu'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['receipts_type_name'];
		$this->base_model->addAcction($this->route,$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['hotel_receipts_type'])
					->where('id',$id)
					->where('id <> 1')
					->update($array);	
		
		$queryDelete = $this->model->table($tb['hotel_receipts_type'])
					->select('group_concat(receipts_type_name) as deletes')
					->where("id in ($id)")
					->find();
