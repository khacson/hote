 ...
 * @copyright 2015
 */
class Historyexportreturn extends CI_Controller {
    private $route;
	private $login;
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/lichsuxuattranncc.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($query as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(8,$i,$item->warehouse_name);
			$sheetIndex->setCellValueByColumnAndRow(9,$i,$cname);
			$sheetIndex->setCellValueByColumnAndRow(10,$i,$item->usercreate);
			$sheetIndex->setCellValueByColumnAndRow(11,$i,date('d-m-Y',strtotime($item->datecreate)));
			$i++;
		}
		$boderthin = "A5:L".($i-1); 
