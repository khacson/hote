 ...
 * @copyright 2015
 */
 class HistoryexportreturnModel extends CI_Model{
	...
	function saves($array){
		...
		$po = "SELECT oc.poid
							FROM `".$tb['hotel_output_createorders_return']."` oc
		...
		if(count($arr_goodsid > 0)){
			$idinsert = $this->model->table($tb['hotel_output_createorders_return'])->insert($array);
			//Them bang chi tiet
			...
			foreach($arr_goodsid as $key=>$sl){
				...
				$array['quantity'] = $sl;
				if(!empty($sl)){
					$this->model->table($tb['hotel_output_return'])->insert($array);
				}	
			}
			return $idinsert;
		}
	...
	function deletes($id, $array){
		...
		foreach($arrID as $k=>$val){
			...
			$this->model->table($tb['hotel_output_return'])
	...
	function edits($array,$id){
		...
		unset($array['todate']);
		if(count($arr_goodsid > 0)){
			$idinsert = $this->model->table($tb['hotel_output_createorders_return'])
							 ->where('id',$id)
							 ->update($array);
			//Them bang chi tiet
			//unset($array['customer_id']);
	...
	function findListID($id){
		...
		$sql = "
				...
				FROM `".$tb['hotel_output_createorders_return']."` AS c
				LEFT JOIN `".$tb['hotel_warehouse']."` w on w.id = c.warehouseid and w.isdelete = 0
