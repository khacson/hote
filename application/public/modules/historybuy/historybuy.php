 ...
 * @copyright 2015
 */
class Historybuy extends CI_Controller {
    private $route;
	private $login;
	...
	function save() {
		...
		$result['status'] =$this->model->saves($array,$idselect);
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/nhaphang.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(4,$i,$item->supplier_name);
			$sheetIndex->setCellValueByColumnAndRow(5,$i,$item->warehouse_name);
			$sheetIndex->setCellValueByColumnAndRow(6,$i,($item->quantity));
			$sheetIndex->setCellValueByColumnAndRow(7,$i,($item->priceone));
			$sheetIndex->setCellValueByColumnAndRow(8,$i,($item->price));
			$sheetIndex->setCellValueByColumnAndRow(9,$i,$item->unit_name);
		...
		//$endxls = '.xls';
        $this->excel_model->exportExcel($objPHPExcel, $versionExcel, "lichsumuahang_".$date.'.xls');
	}
	function getGoods(){
		$id = $this->input->post('id');
