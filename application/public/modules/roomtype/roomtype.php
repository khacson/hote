...
class Roomtype extends CI_Controller {
	...
	function _view(){ 
		...
		$this->site->write('title',getLanguage('loai-phong'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
	...
	function save() {
		...
		$this->base_model->addAcction(getLanguage('loai-phong'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$this->base_model->addAcction(getLanguage('loai-phong'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
