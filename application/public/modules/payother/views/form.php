<div class="row">
	<div class="col-md-12">
		<div class="form-group">
			...
			<div class="col-md-8" >
				<select name="typeid" id="typeid" class="combos-input select2me form-control" data-placeholder="<?=getLanguage('chon-loai-phieu-chi')?>">
					<option value=""></option>
					<?php foreach ($pays as $item) { ?>
						<option <?php if($item->id == $finds->typeid){?> selected <?php }?> value="<?=$item->id;?>"><?=$item->pay_type_name;?></option>
					<?php } ?>
				</select>
			</div>
		</div>
