...
class Customer extends CI_Controller {
	...
	function form(){
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tb['hotel_customer']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function deletes() {
		...
		$checkExitCus = $this->model->table($tb['hotel_output_createorders'])
									  ...
									  ->where('isdelete',0)
									  ->find();
		if(!empty($checkExitCus->customer_id)){
			$result['status'] = -1;	
			$result['csrfHash'] = $token;
			echo json_encode($result); exit;
		}
		...
		$queryDelete = $this->model->table($tb['hotel_customer'])
	...
	function import(){ 
		  ...
		  if(!empty($str_mst)){
			  ...
			  $listCus = $this->model->table($tb['hotel_customer'])
					  ->select('taxcode,id')
		  ...
		  if(!empty($str_tknh)){
			  ...
			  $listBankCode = $this->model->table($tb['hotel_customer'])
					  ->select('bankcode,id')
		  ...
		  //Kiem tra khach hang str_code
		   if(!empty($str_code)){
			  ...
			  $listCode = $this->model->table($tb['hotel_customer'])
