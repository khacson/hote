					...
					<div id="tHeader">    	
						<table id="tbheader" width="100%" cellspacing="0" border="1" >
							...
							<tr>							
								...
								<th id="ord_c.contact_phone"><?=getLanguage('phan-loai-khach-hang')?></th>
								<th id="ord_c.contact_phone"><?=getLanguage('nhom-khach-hang')?></th>
					...
					<div id="gridView">
						<table id="tbbody" width="100%" cellspacing="0" border="1">
							...
							<tr class="row-search">
								...
								<td>
									<select id="customertypeid" name="customertypeid" class="combos" >
								...
								<td>
									<select id="customergroupid" name="customergroupid" class="combos" >
										<?php foreach($customerGroups as $item){?>
											<option value="<?=$item->id;?>"><?=$item->customergroup_name;?></option>
										<?php }?>
									</select>
								</td>
								<td>
...
<script>
	...
	function searchFunction(){
		$("#customer_code,#customer_name,#phone,#fax,#email,#taxcode,#bankcode,#bankname,#contact_name,#contact_phone").keyup(function() {
			searchList();	
		});
	...
	function init(){
		...
		$('#customertypeid').multipleSelect({
			filter: true,
			placeholder:'<?=getLanguage('chon-phan-nhom-khach-hang')?>',
			single: false,
			onClick: function(view){
				searchList();
			}
		});
		$('#customergroupid').multipleSelect({
			filter: true,
			placeholder:'<?=getLanguage('chon-nhom-khach-hang')?>',
			single: false,
			onClick: function(view){
				searchList();
			}
