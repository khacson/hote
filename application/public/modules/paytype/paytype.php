...
class Paytype extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('loai-phieu-chi'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_pay_type']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['pay_type_name'];
		$this->base_model->addAcction(getLanguage('loai-phieu-chi'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['pay_type_name'];
		$this->base_model->addAcction(getLanguage('loai-phieu-chi'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$this->model->table($tb['hotel_pay_type'])
					->where('id',$id)
					->where('id <> 1')
					->update($array);	
		
		$queryDelete = $this->model->table($tb['hotel_pay_type'])
					->select('group_concat(pay_type_name) as deletes')
					->where("id in ($id)")
					->find();
