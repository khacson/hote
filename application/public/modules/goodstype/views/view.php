...
<div class="portlet box blue mtop0">
	<div class="portlet-title">
		<div class="caption">
			 <div class="brc mtop3"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('loai-hang-hoa');?></div>		
		</div>
		<div class="tools">
					...
					<div id="tHeader">    	
						<table id="tbheader" width="100%" cellspacing="0" border="1" >
							...
							<tr>							
								...
								<th id="ord_c.goods_tye_name"><?=getLanguage('loai-hang-hoa')?></th>
								<th id="ord_c.goods_type_group"><?=getLanguage('phan-loai')?></th>
								<th></th>
								<th></th>
							</tr>
						</table>
					...
					<div id="gridView">
						<table id="tbbody" width="100%" cellspacing="0" border="1">
							...
							<tr class="row-search">
								...
								<td></td>
								<td>
									<input type="text" name="goods_tye_name" id="goods_tye_name" class="searchs form-control " />
								</td>
								<td>
									<select id="goods_type_group" name="goods_type_group" class="combos" >
...
<script>
	...
	function init(){
		$('#goods_type_group').multipleSelect({
			filter: true,
			single: false,
			placeholder: '<?=getLanguage('chon-phan-loai')?>',
	...
	function save(){
		...
		if(obj.goods_tye_name == ''){
			warning('<?=getLanguage('loai-hang-khong-duoc-trong');?>'); return false;	
		}
		var token = $('#token').val();
		$('.loading').show();
		$.ajax({
    ...
    function funcList(obj){
		$('.edit').each(function(e){
			$(this).click(function(){ 
				 var goods_tye_name = $('.goods_tye_name').eq(e).html().trim();
				 var id = $(this).attr('id');
				 $('#id').val(id);	
				 $('#goods_tye_name').val(goods_tye_name);
			});
		});	
