...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption caption2">
			 <div class="brc mtop3"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('phieu-thu');?></div>					
		</div>
		<div class="tools">
					...
					<div id="tHeader">    	
						<table id="tbheader" width="100%" cellspacing="0" border="1" >
							...
							<tr>							
								...
								<th id="ord_p.receipts_code"><?=getLanguage('phieu-thu')?></th>
								<th id="ord_p.typeid"><?=getLanguage('loai-phieu-thu')?></th>
					...
					<div id="gridView">
						<table id="tbbody" width="100%" cellspacing="0" border="1">
							<tr class="row-search">
								...
								<td></td>
								<td>
									<input type="text" name="receipts_code" id="receipts_code" class="searchs form-control " />
								</td>
								<td>
									<select id="typeid" name="typeid" class="combos" >
										<?php foreach ($pays as $item) { ?>
											<option value="<?=$item->id;?>"><?=$item->receipts_type_name;?></option>
										<?php } ?>
									</select>
								</td>
								<td>
...
<script>
	...
	function searchFunction(){
		$("#receipts_code,#notes,#amount").keyup(function() {
			searchList();	
		});
	}
	...
	function save(){
		...
		if(obj.typeid == ''){
			warning("<?=getLanguage('chon-loai-phieu-thu')?>"); return false;	
		}
		if(obj.amount == ''){
	...
	function init(){
		...
		$('#typeid').multipleSelect({
			filter: true,
			placeholder:"<?=getLanguage('chon-loai-phieu-thu')?>",
			single: false,
			onClick: function(view) {
				searchList();
			}
    ...
    function funcList(obj){
		$('.edit').each(function(e){
			$(this).click(function(){ 
				 var receipts_code = $('.receipts_code').eq(e).html().trim();
				 ...
				 var id = $(this).attr('id');
				 $('#id').val(id);	
				 $('#receipts_code').val(receipts_code);
				 $('#amount').val(amount);		
				 $('#notes').val(notes);
				 $('#poid').val(poid);
				 $('#formdate').val(datepo);
