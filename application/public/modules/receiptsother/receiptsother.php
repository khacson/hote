 ...
 * @copyright 2015
 */
class Receiptsother extends CI_Controller {
    private $route;
	private $login;
	...
	function form(){
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_receipts']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function deletes() {
		...
		foreach($query as $item){
			$this->model->table($tb['hotel_receipts'])
					->where('id',$item->id)
					->delete();	
			$description = getLanguage('xoa').": ".$item->receipts_code;
	...
	function export(){
		...
		foreach($query as $item){
			...
			$sheetIndex->setCellValueByColumnAndRow(1,$i,$item->receipts_code);
			$sheetIndex->setCellValueByColumnAndRow(2,$i,$item->receipts_type_name);
