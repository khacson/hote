...
class Room extends CI_Controller {
	...
	function _view(){ 
		...
		$this->site->write('title',getLanguage('danh-sach-phong'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_room']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['room_name'];
		$this->base_model->addAcction(getLanguage('danh-sach-phong'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['room_name'];
		$this->base_model->addAcction(getLanguage('danh-sach-phong'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['hotel_room'])
					->select('group_concat(room_name) as bankname')
					->where("id in ($id)")
					->find();
