 ...
 * @copyright 2015
 */
class Ponumber extends CI_Controller {
    private $route;
	private $login;
	...
	function _view(){ 
		...
		$data->superliers = $this->base_model->getSupplier();
	...
	function form(){
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_ponumber']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['ponumber'];
		$this->base_model->addAcction(getLanguage('don-hang'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['ponumber'];
		$this->base_model->addAcction(getLanguage('don-hang'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		#end
		$this->model->table($tb['hotel_ponumber'])
					->where("id in ($id)")
					->update($array);	
		$queryDelete = $this->model->table($tb['hotel_ponumber'])
					->select('group_concat(ponumber) as chungloai')
					->where("id in ($id)")
					->find();
		$description = getLanguage('xoa').": ".$queryDelete->chungloai;
		$this->base_model->addAcction(getLanguage('don-hang'),$this->uri->segment(2),'','',$description);
		$result['status'] = 1;	
		$result['csrfHash'] = $token;
		echo json_encode($result);
