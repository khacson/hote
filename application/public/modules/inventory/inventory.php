...
class Inventory extends CI_Controller {
	...
	function _view(){
		...
		$data->catalogs = $this->base_model->getGoodsType('');
	...
	function changeLoation(){
		...
		$result['status'] = $this->model->table($this->base_model->tb_inventory())->save($id, $array);
	...
	function export(){
        ...
        $fileName = APPPATH . 'Template/tonkho.xls';
        $versionExcel = 'Excel5';
		...
		foreach ($datas as $item) { 
			...
			$sheetIndex->setCellValueByColumnAndRow(5,$i,rNumber($item->exchange_quantity));
			$sheetIndex->setCellValueByColumnAndRow(6,$i,$item->exchange_unit);
			$sheetIndex->setCellValueByColumnAndRow(7,$i,$item->quantitymin);
			$sheetIndex->setCellValueByColumnAndRow(8,$i,$item->branch_name);
			$sheetIndex->setCellValueByColumnAndRow(9,$i,($item->location_name));
			$i++;
		}
		$boderthin = "A2:J".($i-1); 
		...
		//$endxls = '.xls';
        $this->excel_model->exportExcel($objPHPExcel, $versionExcel, "tonkho_".$date.'.xls');
	}
