...
class TranslateModel extends CI_Model
{
	...
	function getLanguage(){
		...
		$query = $this->model->table('hotel_language')
	...
	function saves($array,$id){
		...
		$check = $this->model->table('hotel_translate')
					  ->select('id')
					  ->where('isdelete',0)
		 ...
		 $result = $this->model->table('hotel_translate')->insert($array);	
		 return $result;
	}
	function edits($array,$id){
		...
		$this->model->table('hotel_translate')
					->where('id',$id)
					->update($array);	
		return $id;
