...
class Unit extends CI_Controller {
	...
	function _view(){
		...
		$this->site->write('title',getLanguage('don-vi-tinh'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_unit']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
	...
	function save() {
		...
		$description = getLanguage('them-moi').': '.$array['unit_name'];
		$this->base_model->addAcction(getLanguage('don-vi-tinh'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$description = getLanguage('sua').': '.$array['unit_name'];
		$this->base_model->addAcction(getLanguage('don-vi-tinh'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$checkExitgoods = $this->model->table($tb['hotel_goods'])
									  ...
									  ->where('isdelete',0)
									  ->find();
		if(!empty($checkExitgoods->unitid)){
			$result['status'] = -1;	
			$result['csrfHash'] = $token;
			echo json_encode($result); exit;
		}
		#end
		$this->model->table($tb['hotel_unit'])
					->where("id in ($id)")
					->update($array);	
		
		$queryDelete = $this->model->table($tb['hotel_unit'])
					->select('group_concat(unit_name) as deletes')
					->where("id in ($id)")
					->find();
