...
<div class="portlet box blue">
	...
	<div class="portlet-body">
					...
					<div class="row mtop20">
						<div class="row">
							<div class="form-group">
								<label  class="control-label col-md-3" style="padding-left:30px;">
									<?php if(empty($socode)){?>
									Chọn hàng hóa(<span class="red">*</span>)
									<?php }?>
								</label>
		...
		<input  goodid="<?=$item->id;?>" class="discount" type="hidden" value="<?=$item->discount;?>" />
										...
										?>
										<td style="padding:1px !important;"><input goodid="<?=$item->id;?>" uniqueid="<?=$item->uniqueid;?>" type="number" name="quantity" id="<?=$item->goods_code;?>" placeholder="" class="search form-control quantity text-right fm-number" value="<?=$quantity;?>" style="font-size:12px;"  /></td>
								...
								<tr >
									...
									<td class="ttprice" style="text-align:right; padding-right:10px !important;"><?=fmNumber($tts);?></td>
									<td></td>
									<td></td>
									<td></td>
								</tr>
					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 ">
									<input maxlength="3" type="text" name="vat" id="vat" placeholder="" value="<?=$finds->vat;?>" class="searchs valtotal form-control text-right fm-number" />
								</div>
							</div>
						</div>
					</div><!--E Row-->
							...
							<div class="form-group">
								...
								<div class="col-md-8 ">
									...
									<div class="col-md-5" style="padding:0 !important; ">
										<input style="font-size:12px;" type="text" name="price_prepay" id="price_prepay" placeholder="" class="searchs form-control text-right fm-number" value="<?=fmNumber($finds->price_prepay);?>" />
									</div>
								</div>
							</div>
						</div>
...
<script>
	...
	function gooods(goodsid,code,stype,exchangs,deletes){ 
		...
		$.ajax({
			...
			type: 'POST',
			async: false,
			data: {vat:vat, id:goodsid,code:code,stype:stype,exchangs:exchangs,deletes:deletes,isnew:1},
			success:function(datas){
				var obj = $.evalJSON(datas); 
		...
		$('.discountorder').each(function(ext){
			$(this).on('click',function(){
				...
				$.ajax({
					...
					type: 'POST',
					async: false,
					data: {goodid:goodid, price:price,isnew:1},
					success:function(datas){
						var obj = $.evalJSON(datas); 
