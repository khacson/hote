 ...
 */
 class QuotesModel extends CI_Model{
	...
	function getOderList(){
		...
		$query = $this->model->table($tb['hotel_quotes'])
					  ->select('poid,id')
					  ->where('isdelete',0)
					  ->find_all();
		return $query;
	...
	function saves($uniqueid,$itemList,$array,$isnew){
		...
		if($setuppo == 0){
			...
			$poid = $array['poid']; 
			//Kiem tra ton tai PO
			$checkPOid = $this->model->table($tb['hotel_quotes'])
								 ->select('poid')
								 ->where('poid',$poid )
		...
		$this->model->table($tb['hotel_quotes'])->insert($insert);	
		#end
		$orderid = $this->model->table($tb['hotel_quotes'])	
							   ->select('id')
		...
		foreach($queryTemp as $item){
			...
			$this->model->table($tb['hotel_quotes_detail'])->insert($insertInput);
			$i++;
		}
		#end
		...
		$this->base_model->addAcction(getLanguage('don-dat-hang'),$this->uri->segment(2),'','',$description );	
		#end
	...
	function edits($orderid,$array){
		...
		#end
		//Lay du lieu ban dau
		$queryOld = $this->model->table($tb['hotel_quotes_detail'])
		...
		$this->model->table($tb['hotel_quotes_detail'])
					->where('orderid',$orderid)
					->where('branchid',$branchid)
					->delete();
		...
		$this->model->table($tb['hotel_quotes'])->where('id',$orderid)->update($insert);	
		#end
		#region them bang detail
		$i=1; 
		foreach($queryTemp as $item){
	...
	function findID($id){
		$tb = $this->base_model->loadTable();
		$query = $this->model->table($tb['hotel_quotes_detail'])
					  ->select('*')
					  ->where('id',$id)
					  ->find();
		return $query;
	...
	function findListID($unit){
		...
		$sql = "
			...
			un.unit_name
			FROM `".$tb['hotel_quotes']."` so
			LEFT join `".$tb['hotel_quotes_detail']."` oo on oo.orderid = so.id
	...
	function getNoneCreateorders(){
		...
		WHERE table_name='".$tb['hotel_quotes']."'; 
		";
		$query = $this->model->query($sql)->execute();
		$obj = new stdClass();
		foreach($query as $item){
	...
	function findGoodsByPO($poid){
		...
		$companyid = $this->login->companyid;
		$skey = $this->login->skey;
		$find = $this->model->table($tb['hotel_quotes'])
	...
	function createPoOrder($branchid,$datecreate){
		...
		if($configs['quotes_type'] == 0){		
			...
			if(!empty($checkPOid->poid)){
				...
				if(empty($configs['quotes_default'])){
					$configs['quotes_default'] = '00001';
				}
				$poc = trim($configs['cfdh']).date('ym',strtotime($yearDay)).trim($configs['quotes_default']);
			}
		}
		else{
			$checkPOid = $this->model->table($tb['hotel_quotes'])
			...
			if(!empty($checkPOid->poid)){
				...
				$poc = trim($configs['cfdh']). $this->base_model->createPO($configs['quotes_default'], $poid);
			}
			else{
				if(empty($configs['quotes_default'])){
					$configs['quotes_default'] = '00001';
				}
				$poc = trim($configs['cfdh']).trim($configs['quotes_default']);
			}
		}
		$checkSTT = $this->model->table($tb['hotel_quotes'])
					 ->select('poid,stt')
					 ->where("datepo like '$yearDay%'")
					 ->where('branchid',$branchid)
					 ->where('isdelete',0)
	...
	function getDiscountorderSO($goodid,$poid){
		...
		$sql = "
			SELECT g.id, sat.discount, g.goods_code, DES_DECRYPT(g.goods_name,'".$skey."') as goods_name
			FROM `".$tb['hotel_quotes_detail_order']."`  sat
	...
	function findListUniqueID($orderid){
		...
		$sql = "
			...
			(
				SELECT oc.vat
				FROM `".$tb['hotel_quotes']."` oc
				where oc.id = si.orderid
				and oc.branchid = '$branchid'
				limit 1
			) as vat, si.unitid
			from `".$tb['hotel_quotes_detail']."` si
