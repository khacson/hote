...
class Roomprice extends CI_Controller {
	...
	function _view(){ 
		...
		$this->site->write('title',getLanguage('bang-gia-phong'),true);
        $this->site->render();
	}
	function form(){
		$login = $this->login;
		...
		if(empty($find->id)){
			...
			$find = $this->base_model->getColumns($tbs['hotel_roomprice']);
		}
		$data = new stdClass();
        $result = new stdClass();
		$data->finds = $find;  
		...
		$data->listDetails = $this->model->findListDetail($find->id);
	...
	function save() {
		...
		$valueItem = json_decode($this->input->post('valueItem'),true);
		...
		$result['status'] =$this->model->saves($array,$valueItem,$issave);
		
		
		$description = getLanguage('them-moi').': '.$array['roomprice_name'];
		$this->base_model->addAcction(getLanguage('bang-gia-phong'),$this->uri->segment(2),'','',$description);
		$result['csrfHash'] = $token;
		echo json_encode($result);
	}
	function edit() {
		...
		$array['branchid'] = $this->login->branchid;
		$result['status'] =$this->model->edits($array,$id,$valueItem,$issave);
		$arr_log['func'] = $this->uri->segment(2);
		$description = getLanguage('sua').': '.$array['roomprice_name'];
		$this->base_model->addAcction(getLanguage('bang-gia-phong'),$this->uri->segment(2),json_encode($acction_before),json_encode($array),$description);
	...
	function deletes() {
		...
		$queryDelete = $this->model->table($tb['hotel_roomprice'])
					->select('group_concat(roomprice_name) as bankname')
					->where("id in ($id)")
					->find();
		
		$this->model->table($tb['hotel_roomprice'])
					->select('group_concat(roomprice_name) as bankname')
					->where("id in ($id)")->update($array);
