...
<div class="portlet box blue mtop0">
	<div class="portlet-title">
		<div class="caption">
			 <div class="brc mtop3"><i class="fa fa-bars"></i> <?=getLanguage('tim-thay');?> <span class="semi-bold viewtotal">0</span> <?=getLanguage('bang-gia-phong');?></div>		
		</div>
		<div class="tools">
					...
					<div id="tHeader">    	
						<table id="tbheader" width="100%" cellspacing="0" border="1" >
							...
							<tr>							
								...
								<th id="ord_rt.roomprice_name"><?=getLanguage('ten-bang-gia')?></th>
					...
					<div id="gridView">
						<table id="tbbody" width="100%" cellspacing="0" border="1">
							...
							<tr class="row-search">
								...
								<td></td>
								<td>
									<input type="text" name="roomprice_name" id="roomprice_name" class="searchs form-control " />
								</td>
								<td>
  ...
  <div class="modal-dialog" style="width:1100px;">
    ...
    <div class="modal-content ">
      ...
      <div class="modal-footer">
		 <span style="color:red;"><i>*** Copy: Lấy giá phòng thứ nhất copy cho các phòng còn lại</i></span>
		 <button id="actionSave" type="button" class="btn btn-info" ><i class="fa fa-save" aria-hidden="true"></i>  <?=getLanguage('luu');?></button>
		 <button id="actionSaveCopy" type="button" class="btn btn-info" ><i class="fa fa-save" aria-hidden="true"></i>  <?=getLanguage('copy');?></button>
...
<script>
	...
	$(function(){
		...
		$('#actionSaveCopy').click(function(){
			save(1);
		});	
		searchFunction();
	});
	function searchFunction(){
		$("#roomprice_name,#description").keyup(function() {
			searchList();	
		});
	}
	...
	function save(issave){
		...
		if(obj.roomprice_name == ''){
			warning('<?=getLanguage('ten-bang-gia-khong-duoc-trong');?>'); return false;	
		}
		var token = $('#token').val();
		var valueItem = getValueItem();
		$('.loading').show();
		$.ajax({
			url : controller + func,
			type: 'POST',
			async: false,
			data: {search:search , id:id, issave:issave,valueItem:valueItem},
			success:function(datas){
				var obj = $.evalJSON(datas); 
    ...
    function funcList(obj){
		$('.edit').each(function(e){
			$(this).click(function(){ 
				 /*var roomprice_name = $('.roomprice_name').eq(e).html().trim();
				 var bank_name = $('.bank_name').eq(e).html().trim();
				 var description = $('.description').eq(e).html().trim();
				 var id = $(this).attr('id');
				 $('#id').val(id);	
				 $('#roomprice_name').val(roomprice_name);
