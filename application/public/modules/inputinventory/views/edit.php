...
<div class="portlet box blue">
	<div class="portlet-title">
		<div class="caption mtop8">
			...
			<?=getLanguage('sua-phieu-nhap-kho');?></b>
		</div>
		<div class="tools">
						...
						<div class="col-md-9" style="">
							<select id="ponumberid" name="ponumberid" class="select2me form-control" data-placeholder="<?=getLanguage('chon-don-hang');?>">
								...
								<?php foreach($ponumbers as $item){?>
									<option <?php if($finds->ponumber == $item->id){?> selected <?php }?> value="<?=$item->id;?>"><?=$item->ponumber;?></option>
								<?php }?>
							</select>
						</div>
					</div>
			...
			<div class="row mtop10">
				...
				<div class="col-md-4">
					<div class="form-group">
						...
						<div class="col-md-8 ">
							<input type="text" name="description" id="description" value="<?=$finds->description;?>" class="searchs form-control tab-event" maxlength="250"/>
						</div>
					</div>
				</div>
			</div>
					  ...
					  <div class="row">
						...
						<div class="col-md-12 mtopline">
							...
							<div class="col-md-6">
								<div class="row">
									<div class="col-md-6">
										<input type="text" name="discount" placeholder="" id="discount" class="form-input fm-number form-control text-right" value="<?=$finds->discount;?>" />
									</div>
									<div class="col-md-6">
										<div class="row">
											<select class="form-control select2me" id="input_discount_type" name="input_discount_type" >
												<option <?php if($finds->discount_type == 1){?> selected <?php }?> value="1"><?=configs()['currency'];?></option>
												<option <?php if($finds->discount_type == 2){?> selected <?php }?> value="2">%</option>
											</select>
										</div>
									</div>
								</div>
						...
						<div class="col-md-12 mtopline">
							...
							<div class="col-md-6">
								<div class="row">
									<div class="col-md-6">
										<input type="text" name="adjustment" id="adjustment" class="form-input form-control text-right" value="<?=$finds->adjustment;?>" />
									</div>
									<div class="col-md-6"></div>
								</div>
							</div>
							<div class="col-md-3 text-right"><div class="row" id="total-adjustment"><?=$finds->adjustment;?></div></div>
						</div><!--E Row-->
						<div class="col-md-12 mtopline">
							<div class="col-md-3">
							...
							<div class="col-md-6">
								<div class="row">
									<div class="col-md-6">
										<input type="text" name="vat" placeholder="" id="vat" class="form-input form-control fm-number text-right" value="<?=$finds->vat;?>" />
									</div>
									<div class="col-md-6"></div>
								</div>
							</div>
							<div class="col-md-3 text-right"><div class="row" id="total-vat"><?=$finds->vat_value;?></div></div>
						</div><!--E Row-->
						...
						<div class="col-md-12 mtopline">
							...
							<div class="col-md-6">
								<div class="row">
									<div class="col-md-6">
										<input type="text" name="price_prepay" placeholder="" id="price_prepay" class="form-input fm-number form-control text-right" value="<?=$finds->price_prepay;?>" />
									</div>
									<div class="col-md-6">
										<div class="row">
...
<input type="hidden" name="unit" placeholder="" id="unit" value="<?=$uniqueid;?>" />
<input type="hidden" name="poid" placeholder="" id="poid" value="<?=$poid;?>" />
...
<script>
	...
	$(function(){
		...
		$('#viewprintpc').click(function(){
			var unit = $('#poid').val();  
			if(unit == ''){ return false;}
			printpc(0,unit);
		});
		$('#viewprintpc2').click(function(){
			var unit = $('#poid').val(); 
			if(unit == ''){ return false;}
			printpc(0,unit);
		});
	...
	function save(func,id){ 
		...
		$.ajax({
			...
			success:function(datas){
				...
				if(obj.status == 0){
					...
					if(id == 0){
						...
						success("<?=getLanguage('sua-nhap-kho-thanh-cong');?> "+ obj.poid);
					}
					else{ //poid
						$('#unit').val(obj.status);
						$('#poid').val(obj.poid);
			...
			error : function(){
				$(".loading").hide();
				error("<?=getLanguage('sua-nhap-kho-khong-thanh-cong');?>"); return false;
			}
		});
	}
	...
	function printpc(id,unit){
		var token = $('#token').val();
		$.ajax({
			url : controller + 'getDataPrintPC?unit='+unit,
			type: 'POST',
			async: false,
			data: {csrf_stock_name:token,id:id},
			success:function(datas){
	...
	function updateDataTemp(goodid){
		...
		$.ajax({
			...
			type: 'POST',
			//async: false,
			data: {goodid:goodid, priceone:priceone,quantity:quantity,discount:discount,xkm:xkm,isnew:isnew,vat:vat,unitid:unitid},
			success:function(datas){
				var object = $.evalJSON(datas); 
