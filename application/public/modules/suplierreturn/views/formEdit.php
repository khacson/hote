					...
					<div class="row mtop20">
							<div class="row">
								<div class="form-group">
									...
									<div style="margin-top:-43px;" class="col-md-2" id="viewImg" >
						...
						<table class="inputgoods">
							...
							<tbody class="gridView">
								...
								<tr class="tgrid">
									...
									<td width="100"><input goodid="<?=$item->goodsid;?>" type="text" name="priceone" id="priceone" placeholder="" class="search form-control priceone text-right fm-number" value="<?=$item->priceone;?>" /></td>
			  ...
			  <div class="col-md-4" style="margin-left:1%; width:33%; border-left:1px dashed #c3cfd7;  margin-bottom:10px; padding-left:5px;">
						...
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input type="text" value="<?=$dataList[0]->poid;?>" name="poid" id="poid" <?php if(empty($setuppo)){?> readonly <?php }?> class="form-control searchs" />
								</div>
							</div>
						</div>
					</div>
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="col-md-6">
								<label>
									<input value="0" id="customer_type_0" type="radio" <?php if($dataList[0]->customer_type == 0){?> checked <?php }?> name="customer_type" />
									Khách hàng lẻ
								</label>
							</div>
							<div class="col-md-6">
								&nbsp;&nbsp;<label>
									<input value="1" id="customer_type_1" type="radio" <?php if($dataList[0]->customer_type == 1){?> checked <?php }?> name="customer_type" />
									Khách đại lý
								</label>
							</div>
						</div>
					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input type="text" name="customer_name" id="customer_name" placeholder="" value="<?=$dataList[0]->customer_name;?>" class="searchs form-control" />
								</div>
							</div>
						</div>
					</div>
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input type="text" name="customer_phone" id="customer_phone" placeholder="" value="<?=$dataList[0]->customer_phone;?>" class="searchs form-control" />
								</div>
							</div>
						</div>
					</div>
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input type="text" name="customer_address" id="customer_address" placeholder="" value="<?=$dataList[0]->customer_address;?>" class="searchs form-control" />
								</div>
							</div>
						</div>
					</div>
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input type="text" name="customer_email" id="customer_email" placeholder="" value="<?=$dataList[0]->customer_email;?>" class="searchs form-control" />
								</div>
							</div>
						</div>
					</div>
									...
									<span id="loadCustomer">
										<select id="customerid" name="customerid" class="combos" >
											...
											<?php foreach($customers as $item){?>
												<option <?php if($dataList[0]->customer_id == $item->id){?> selected <?php }?> value="<?=$item->id;?>"><?=$item->customer_name;?></option>
											<?php }?>
										</select>
									</span>
								</div>
								...
								<div class="col-md-8 pright5">
									<select id="employeeid" name="employeeid" class="combos" >
										...
										<?php foreach($employeesale as $item){?>
											<option <?php if($dataList[0]->employeeid == $item->id){?> selected <?php }?>
							...
							<div class="form-group">
								<label class="control-label col-md-4">Ngày giao</label>
								 <div class="col-md-8 input-group date date-picker" data-date-format="dd-mm-yyyy">
									<?php 
									if(!empty($dataList[0]->deliverydate) && $dataList[0]->deliverydate != '0000-00-00'){
										 $deliverydate = date('d-m-Y',strtotime($dataList[0]->deliverydate));
									}
									else{
										$deliverydate = '';
									}
									?>
									<input value="<?=$deliverydate;?>" type="text" id="deliverydate" placeholder="dd-mm-yyyy" name="deliverydate" class="form-control searchs" >
					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input type="text" name="place_of_delivery" id="place_of_delivery" placeholder="Địa điểm giao hàng" value="<?=$dataList[0]->place_of_delivery;?>" class="searchs form-control" />
								</div>
							</div>
						</div>
					</div>
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<textarea class="searchs textarea" name="description" id="description"><?=$dataList[0]->description;?></textarea>
								</div>
							</div>
						</div>
					</div><!--E Row-->
									...
									<span id="loadWarehouse">
										<select id="warehouseid" name="warehouseid" class="combos" >
											...
											<?php foreach($warehouses as $item){?>
												<option 
												<?php if($dataList[0]->warehouseid == $item->id){?> selected <?php }?>
							...
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<div class="col-md-4 pleft0">
										...
										<input  <?php if($dataList[0]->payments == 1){?> checked <?php }?> type="radio" id="payments_1" name="payments" value="1"  />
										Tiền mặt</label>
									</div>
									<div class="col-md-4">
										<label class="control-label">
										<input <?php if($dataList[0]->payments == 2){?> checked <?php }?> type="radio"  id="payments_2" name="payments" value="2"  />
										CK</label>
									</div>
									<div class="col-md-4">
										<label class="control-label">
										<input type="radio" <?php if($dataList[0]->payments == 3){?> checked <?php }?> id="payments_3" name="payments" value="3"  />
										Thẻ</label>
									</div>
								</div>
							</div>
					...
					<div class="row mtop10">
						<div class="col-md-12">
							<div class="form-group">
								...
								<div class="col-md-8 pright5">
									<input value="<?=$dataList[0]->vat;?>" maxlength="3" type="text" name="vat" id="vat" placeholder="" class="searchs form-control text-right fm-number" />
								</div>
							</div>
						</div>
					</div><!--E Row-->
...
<script>
	...
	$(function(){
		...
		stt();
		var isorder = 0;
		var type = '<?=$dataList[0]->customer_type;?>';
		...
		$('#save2,#save').click(function(){
			save('edit','');
		});
		$('#print2,#print').click(function(){
			save('edit','print');
		});
		...
		$.ajax({
			...
			success:function(datas){
				...
				if(poid != ''){
					...
					if(parseInt(finds.customer_type) == 0){
	...
	function setDefault(){
		//$('#customer_type_0').attr('checked',true);
		customerType(1);
		$('#poid').val('');
	...
	function init(){
		...
		$( "#goodsid" ).keypress(function(e){
			if(e.keyCode == 13){ //Scan
				...
				var add  = 0;
				$(".goodscode").each(function(item){  
					var iCode = $.trim($(".goodscode").eq(item).val());
					var quantity =  $.trim($(".quantity").eq(item).val());
					quantity = quantity.replace(/[^0-9+\-Ee.]/g, '');
					quantity = parseInt(quantity) + 1;
					if(code == iCode){ 
						add = 1;
						//Gia thanh tien
						$.trim($(".quantity").eq(item).val(quantity));
						var priceone = $('.priceone').eq(item).val();
						priceone = priceone.replace(/[^0-9+\-Ee.]/g, '');
						priceone = parseInt(priceone);
						$('.price').eq(item).val(formatOne(priceone*quantity));
						//So luong tong
					}
					else{
						add  = 0;
		...
		$( "#goodsid" ).autocomplete({
			...
			select: function( event, ui ){ 
				...
				if(typeof inputList[goodsid] === 'undefined'){
					...
					//Cộng dồn số lượng khi san lai
					var quantity = thisQuantity.val(); 
					quantity = quantity.replace(/[^0-9+\-Ee.]/g, '');
					quantity = parseInt(quantity);
					$('#'+goodsid).parent().parent().find('.quantity').val(quantity+1);
					//Gia
					var priceone = $('#'+goodsid).parent().parent().find('.priceone').val();
					priceone = priceone.replace(/[^0-9+\-Ee.]/g, '');
					priceone = parseInt(priceone);
					var priceTotal = priceone * (quantity+1);
					$('#'+goodsid).parent().parent().find('.buyprice').val(formatOne(priceTotal));
					$('#goodsid').val('');
				}
				calInputTotal();
				return false;
