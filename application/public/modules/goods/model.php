 ...
 class GoodsModel extends CI_Model{
	...
	function getList($search,$page,$rows){
		...
		$sql = "
				...
				c.goods_type, c.id, c.madein, c.discountsales, c.unitid, c.sale_price, c.buy_price, c.companyid, c.description,
				c.goods_name, vat,
				p.goods_tye_name,ut.unit_name, utc.unit_name as unit_name_active, c.exchange_unit, 
				...
				LEFT JOIN `$tb_unit` ut on ut.id = c.unitid
				LEFT JOIN `$tb_unit` utc on utc.id = c.unitid_active
				WHERE c.isdelete = 0
				AND p.goods_type_group = 1
				$searchs 
				";
	...
	function saves($array,$materials){
		...
		if(empty($array['unitid_active'] )){
			$array['unitid_active']  = $array['unitid'];
		}
